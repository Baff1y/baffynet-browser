...
// Функция для получения/сохранения масштаба
function getZoomLevel(tabId) {
  const zoomLevels = JSON.parse(localStorage.getItem('zoomLevels') || '{}')
  return zoomLevels[tabId] || 1.0
}

function setZoomLevel(tabId, level) {
  const zoomLevels = JSON.parse(localStorage.getItem('zoomLevels') || '{}')
  zoomLevels[tabId] = Math.max(0.25, Math.min(5.0, level)) // Ограничиваем масштаб
  localStorage.setItem('zoomLevels', JSON.stringify(zoomLevels))
  
  // Применяем масштаб к webview
  const webview = document.querySelector(`webview[data-tab-id="${tabId}"]`)
  if (webview) {
    webview.setZoomLevel(zoomLevels[tabId])
  }
}
...
function createNewTab(url) {
  ...
  tab.innerHTML = `
    ...
    <span class="tab-title">${finalUrl.includes('baffynet.rf.gd') ? 'Newtab' : 'Новая вкладка'}</span>
  ...
  // Применяем сохраненный масштаб при создании вкладки
  const zoomLevel = getZoomLevel(tabId)
  ...
  // Обработчик колесика мыши с Ctrl
  webview.addEventListener('wheel', (e) => {
    if (e.ctrlKey) {
      e.preventDefault()
      const delta = e.deltaY > 0 ? -0.1 : 0.1
      const currentZoom = getZoomLevel(tabId)
      const newZoom = currentZoom + delta
      setZoomLevel(tabId, newZoom)
    }
  })
...
if (window.electronAPI) {
  ...
  window.electronAPI.onOpenPrivacyNet(() => {
    console.log('Opening PrivacyNet...')
  })

  // Обработчики изменения масштаба
  window.electronAPI.onChangeZoomLevel((event, {tabId, delta}) => {
    const currentZoom = getZoomLevel(tabId)
    const newZoom = currentZoom + delta
    setZoomLevel(tabId, newZoom)
  })

  window.electronAPI.onResetZoomLevel((event, tabId) => {
    setZoomLevel(tabId, 1.0)
  })
}
...
document.addEventListener('wheel', (e) => {
  if (e.ctrlKey) {
    ...
    const activeWebview = document.querySelector('webview[style*="display: flex;"]')
    if (activeWebview) {
      const tabId = activeWebview.dataset.tabId
      const delta = e.deltaY > 0 ? -0.1 : 0.1
      const currentZoom = getZoomLevel(tabId)
      const newZoom = currentZoom + delta
      setZoomLevel(tabId, newZoom)
